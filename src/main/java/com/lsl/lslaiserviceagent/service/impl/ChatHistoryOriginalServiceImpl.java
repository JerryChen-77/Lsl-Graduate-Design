...
import com.lsl.lslaiserviceagent.mapper.ChatHistoryOriginalMapper;
...
public class ChatHistoryOriginalServiceImpl extends ServiceImpl<ChatHistoryOriginalMapper, ChatHistoryOriginal> implements ChatHistoryOriginalService {
    ...
    @Override
    public boolean addOriginalChatMessage(Long chatId, String message, String messageType, Long userId) {
        // 参数校验
        ThrowUtils.throwIf(chatId == null || chatId <= 0, ErrorCode.PARAMS_ERROR, "应用 ID不能为空");
    ...
    public boolean deleteByChatId(Long chatId) {
        ThrowUtils.throwIf(chatId == null || chatId <= 0, ErrorCode.PARAMS_ERROR, "对话 ID不能为空");
    ...
    @Override
    public int loadOriginalChatHistoryToMemory(Long chatId, MessageWindowChatMemory chatMemory, int maxCount) {
        try{
            // 1. 查询历史记录，考虑边缘记录类型
            List<ChatHistoryOriginal> originalHistoryList = queryHistoryWithEdgeCheck(chatId, maxCount);
            ...
            log.info("成功为 chatId: {} 加载 {} 条历史对话", chatId, loadedCount);
            return loadedCount;
        } catch (Exception e) {
            log.error("加载历史对话失败，chatId: {}，error: {}", chatId, e.getMessage(), e);
     ...
     * @return 历史记录列表
     */
    private List<ChatHistoryOriginal> queryHistoryWithEdgeCheck(Long chatId, int maxCount) {
        // 1. 首先检查总记录数
        QueryWrapper countQueryWrapper = QueryWrapper.create()
                .eq(ChatHistoryOriginal::getChatId, chatId);
    ...
    private int loadMessagesToMemory(List<ChatHistoryOriginal> originalHistoryList, MessageWindowChatMemory chatMemory) {
        ...
        for(ChatHistoryOriginal history : originalHistoryList) {
            ...
            switch (messageTypeEnum) {
                ...
                default -> log.error("未知消息类型: {}", messageType);
            }
        }
        return loadedCount;
    }
